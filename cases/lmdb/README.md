...
clojure -M:jepsen set kill sync=off    # MDB_NOSYNC — read Honest limits first
```

And the fault a `kill -9` cannot be — on Linux, with lazyfs, from the repository root:

```sh
cases/power-off lmdb set power-off           # fsyncs on commit -> survives
cases/power-off lmdb set power-off sync=off  # MDB_NOSYNC       -> loses writes
cases/power-off lmdb set kill     sync=off   # same store       -> and passes
```
...
results are written under `store/` in Jepsen's normal layout; the environments
the runs are made against are under `jepsen-data/`, wiped fresh before each one
— a crash test means nothing if what it recovers turns out to be the previous
run's.
